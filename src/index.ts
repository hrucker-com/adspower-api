namespace AdsPowerInterface {
    ...
    export interface fingerprint_config {
...
class AdsPowerLocalAPI {
     ...
     */
    constructor(apiPort: number = 50325) {
    ...
    API = {
         ...
         */
        status: (): Promise<any> => this._sendRequest('/status', 'GET'),
    };
    ...
    Profiles = {
        ...
        update: (
            ...
            return this._sendRequest(`${this.basePath}user/update`, 'POST', {
            user_id: profileId,
            ...profileData,
            });
        },
    ...
    Groups = {
        ...
        create: (groupName: string, remark?: string): Promise<any> => {
            const payload: { group_name: string; remark?: string } = { group_name: groupName };
        ...
        edit: (groupId: string, newGroupName: string, remark?: string): Promise<any> => {
            const payload: { group_id: string; group_name: string; remark?: string } = {
                group_id: groupId,
         ...
         * @returns A promise resolving to the response data.
         */
        list: (groupName?: string, page: number = 1, pageSize: number = 10): Promise<any> => {
            const params: { group_name?: string; page: number; page_size: number } = { page, page_size: pageSize };
    ...
    Extensions = {
         ...
         * @returns {Promise<any>} The API response.
         */
        list: (page: number = 1, pageSize: number = 10): Promise<any> =>
            this._sendRequest(`${this.basePath}application/list`, 'GET', { page, page_size: pageSize }),
    };
     ...
     * @private
     */
    private async _sendRequest(endpoint: string, method: string, data: Record<string, any> = {}): Promise<any> {
